...
## Report 32 (version 2) for publication central analysis
if(1)
{
  ...
  args <- data.table(
    ...
    script_file= 'scripts/stan-make-data-usa-v110.r',
    ...
    seed= 42,
    chain= 1,
    job_tag= '40states_tau10_schoolbound6_v110',
    countries = countries,			
    cmdstan = 1L,
...
## Report 32 (version 2) sensitivity analysis, assume 50% lower contacts compared to Zhang et al. during school period from/to 0-14
if(0)
{
  countries <- "AL,AZ,CA,CO,CT,DC,DE,FL,GA,IA,ID,IL,IN,KS,KY,LA,MA,MD,MI,MN,MO,MS,NC,ND,NH,NJ,NM,NV,NYC,OK,OR,PA,RI,SC,TN,TX,UT,VA,WA,WI"
  n_countries <- length(unlist(strsplit(countries,',')))
  ...
  args <- data.table(
    ...
    seed= 42,
    chain= 1,
    job_tag= '40states_tau05_schoolbound6_v110',
    countries = countries,			
    cmdstan = 1L,
...
## Report 32 (version 2) sensitivity analysis, assume 100% more contacts compared to Zhang et al. during school period from/to 0-14
if(0)
{
  countries <- "AL,AZ,CA,CO,CT,DC,DE,FL,GA,IA,ID,IL,IN,KS,KY,LA,MA,MD,MI,MN,MO,MS,NC,ND,NH,NJ,NM,NV,NYC,OK,OR,PA,RI,SC,TN,TX,UT,VA,WA,WI"
  n_countries <- length(unlist(strsplit(countries,',')))
  ...
  args <- data.table(
    ...
    seed= 42,
    chain= 1,
    job_tag= '40states_tau20_schoolbound6_v110',
    countries = countries,			
    cmdstan = 1L,
...
## Report 32 (version 2) sensitivity analysis, using Levin et al. prior version 5
if(0)
{
  countries <- "AL,AZ,CA,CO,CT,DC,DE,FL,GA,IA,ID,IL,IN,KS,KY,LA,MA,MD,MI,MN,MO,MS,NC,ND,NH,NJ,NM,NV,NYC,OK,OR,PA,RI,SC,TN,TX,UT,VA,WA,WI"
  n_countries <- length(unlist(strsplit(countries,',')))
  ...
  args <- data.table(
    ...
    seed= 42,
    chain= 1,
    job_tag= '40states_tau10_schoolbound6_Levinv5_v110',
    countries = countries,			
    cmdstan = 1L,
    multiplier_cntct_school_closure = 1,
    with_forecasts = 0,
...
## Report 32 (version 2) sensitivity analysis, assume attack rates among school-aged children and teens (xi) of 2
if(0)
{
  countries <- "AL,AZ,CA,CO,CT,DC,DE,FL,GA,IA,ID,IL,IN,KS,KY,LA,MA,MD,MI,MN,MO,MS,NC,ND,NH,NJ,NM,NV,NYC,OK,OR,PA,RI,SC,TN,TX,UT,VA,WA,WI"
  n_countries <- length(unlist(strsplit(countries,',')))
  ...
  args <- data.table(
    ...
    seed= 42,
    chain= 1,
    job_tag= '40states_tau10_schoolbound2_v110',
    countries = countries,			
    cmdstan = 1L,
    multiplier_cntct_school_closure = 1,
    with_forecasts = 1,
...
## Report 32 (version 2) sensitivity analysis, assume attack rates among school-aged children and teens (xi) of 10
if(0)
{
  countries <- "AL,AZ,CA,CO,CT,DC,DE,FL,GA,IA,ID,IL,IN,KS,KY,LA,MA,MD,MI,MN,MO,MS,NC,ND,NH,NJ,NM,NV,NYC,OK,OR,PA,RI,SC,TN,TX,UT,VA,WA,WI"
  n_countries <- length(unlist(strsplit(countries,',')))
  ...
  args <- data.table(
    ...
    seed= 42,
    chain= 1,
    job_tag= '40states_tau10_schoolbound10_v110',
    countries = countries,			
    cmdstan = 1L,
    multiplier_cntct_school_closure = 1,
    with_forecasts = 1,
...
for(i in seq_len(nrow(args)))
{
  ...
  {		 
    ...
    stopifnot(is.numeric(tmp))
    cmd <- paste0( cmd, paste0('xargs -P ',tmp,' -n 1 -I {} ') )
    tmp <- paste0('Rscript ', file.path(args$source_dir[i],paste0(gsub('.r','',args$script_generate_quantities_file[i],fixed=TRUE),'-forecast-v110.r')),
                  ' -indir.results "$JOB_DIR2"',
                  ' -location.index {}',
                  ' -with.flow 1',
  ...
  if(i==1)
  {
    ...
    tmp <- paste0('Rscript ', file.path('$SCRIPT_DIR','scripts','post-processing-deathsage-casesage-plot-v110.R'),
                  ' -stanModelFile $STAN_MODEL_FILE -out_dir $OUT_DIR -job_tag $JOB_TAG -overwrite $OVERWRITE -with_forecast $WITH_FORECAST')
    cmd2 <- paste0(cmd2,tmp,'\n')
