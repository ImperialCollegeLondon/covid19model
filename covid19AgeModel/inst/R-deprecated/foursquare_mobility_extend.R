...
for (state in unique_states){
  subset_states <- foursquare_data[which(foursquare_data$code == state),]
  add_dates <- max(subset_states$date) + 1:14
  padded <- data.frame("sub_region_1" = rep(subset_states$sub_region_1[1], 14),
                       "date" = add_dates,
                       "shops.services" = rep(0.2, 14),
                       "grocery" = rep(0.2, 14),
                       "outdoors.recreation" = rep(0.2, 14), 
                       "gasstations" = rep(0.2, 14),
                       "professional" = rep(0.2, 14),
                       "fast.food.restaurants" = rep(0.2, 14),
                       "medical" = rep(0.2, 14),
                       "code" = rep(subset_states$code[1], 14))
  padded_tmp <- bind_rows(subset_states, padded)
  padded_subset <- bind_rows(padded_subset, padded_tmp)
}
foursquare_data <- padded_subset
...
g <- ggplot(foursquare_data_long) +
  ...
  scale_x_date(date_breaks = "2 weeks", labels = date_format("%e %b"), 
               limits = c(as.Date("2020-02-25"), max(foursquare_data_long$date))) + 
