...
args$file_age_ifr <- file.path(args$indir,'data','ifr-by-age-prior_Levin_raw_201117.csv')

#
# Read and clean Levin et al. Supplementary material
ifr_lognormal = as.data.table(read.csv( args$file_age_ifr ))
tmp1 = select(ifr_lognormal, c("Age", "M_IFR", "CL_CI", "CU_CI"))
tmp1 = subset(tmp1, !Age %in% c("0-34", "95+"))
tmp1[, log_ifr_mean := log(M_IFR/100) ]
tmp1[, log_ifr_cl := log(CL_CI/100) ]
tmp1[, log_ifr_cu := log(CU_CI/100) ]
tmp1[, Age := as.numeric(Age)]
setnames(tmp1, "Age", "age")
tmp1 = select(tmp1, -c("M_IFR", "CL_CI", "CU_CI"))

#
# extrapolate straight line for age 0
x = tmp1$age
...
ifr.by.age = as.data.table(aggregate_ifr_data_by_age(path_to_file_pop_us = GFNAME_us_population, ifr.by.age = tmp, max_age = 95))
...
#
# save
write.csv(ifr.by.age, file = file.path(args$indir, "data", 'ifr-by-age-prior_Levin_201117.csv'), row.names = F)
