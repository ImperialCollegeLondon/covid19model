functions {
        ...
        )
    {
        ...
        for (t in (N0+1):N2)
        {
            ...
            row_vector[A] prop_susceptibleByAge = rep_row_vector(1.0, A) - acquire_immunity * (rep_row_vector(1.0, t-1) * E_casesByAge[1:(t-1),:] ./ popByAge_abs_local);
        ...
        )
    {
        ...
        for(a in 1:A)
        {
            ...
            prop_susceptibleByAge[2:N2,a] = acquire_immunity * cumulative_sum( E_casesByAge_local[1:(N2-1),a] ) / popByAge_abs_local[a];
            for( t in 1:N2)
            {
...
parameters {
  ...
  real<lower=0,upper=1> acquire_immunity;
}
...
model {
  ...
  sd_log_ifr_age_rnde_mid1 ~ exponential(1);
  sd_log_ifr_age_rnde_mid2 ~ exponential(1);
  sd_log_ifr_age_rnde_old ~ exponential(1);
  log_ifr_age_rnde_mid1 ~ normal(0, sd_log_ifr_age_rnde_mid1);
  log_ifr_age_rnde_mid2 ~ normal(0, sd_log_ifr_age_rnde_mid2);
  log_ifr_age_rnde_old ~ normal(0, sd_log_ifr_age_rnde_old);
  ...
  log_reltransmissibility_age_reduced[2] ~ normal(0, sd_log_reltransmissibility_age);
