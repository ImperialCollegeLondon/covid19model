functions {
                          ...
                          )
  {
    ...
    matrix[A,A] impact_intv[N2];
    
    // define multipliers for contacts in each location 
    for(t in 1:N2){
      impact_intv[t] = exp( log_rho0 + rep_matrix(beta, A) .* covariates_local[t] );
    }
                              ...
                              )
  {
    ...
    matrix[A,A] impact_intv[N2] = country_impact(R0_local,
                                               beta, 
                                               N2,
                                               A,
                                               avg_cntct_local,
    ...
    for (t in (N0+1):N2)
    {
      ...
      if(r_in(t, wkend_idx_local) == 1){
        E_casesByAge[t] = tmp_row_vector_A * (cntct_weekends_mean_local .* impact_intv[t]);
      }else{
        E_casesByAge[t] = tmp_row_vector_A * (cntct_weekdays_mean_local .* impact_intv[t]);
                              ...
                              )
  {
    ...
    // expected new cases by calendar day, age, and location under self-renewal model
    // and a container to store the precomputed cases by age
    matrix[N2,A] E_casesByAge = country_EcasesByAge(R0_local, e_cases_N0_local, beta,
                              N0, N2, A, SI_CUT, wkend_idx_local,
                              avg_cntct_local, covariates_local,
...
data {
  ...
  matrix[A,A] covariates[N2,M]; // predictors for fsq contacts by age
  // data by age
...
parameters {
  ...
  vector[A] beta; // regression coefficients for time varying multipliers on contacts 
...
generated quantities {
  ...
  {
    matrix[N2,A] tmp;
    matrix[A,A] impact_intv[N2] = country_impact(R0[m],
                                               beta,
                                               N2,
                                               A,
                                               avg_cntct[m],
    ...
    
     {
       matrix[N2,A] prop_susceptibleByAge;
        for(a in 1:A){
            ...
            prop_susceptibleByAge[2:N2,a] = cumulative_sum( E_casesByAge[m][1:(N2-1),a] ) / popByAge_abs[m,a];
            for( t in 1:N2)
            {
                if( prop_susceptibleByAge[t,a]>1 )
                {
                    prop_susceptibleByAge[1,a]= 1.;
                }
            }
        }
        ...
        for(t in 1:N2)
        {
            ...
            {
                RtByAge[m][t,:] = ( cntct_weekends_mean[m] * (RtByAge[m][t,:])' )' .* impact_intv[t,:];
            }
            else
            {
                RtByAge[m][t,:] = ( cntct_weekdays_mean[m] * (RtByAge[m][t,:])' )' .* impact_intv[t,:];
            }
        }
    }
    ...
    // for(a in 1:A){
    //   prop_susceptibleByAge[m][:,a] = rep_vector(1.0, N2) - (cumulative_sum(E_casesByAge[m][:,a]) / popByAge_abs[m,a]);
    // }
   }
}
