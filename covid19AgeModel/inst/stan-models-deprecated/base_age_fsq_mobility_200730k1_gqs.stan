functions {
        ...
        )
    {
        ...
        matrix[N2,A] impact_intv_tmp;
        matrix[A,A] impact_intv[N2];
        vector[N2] upswing_timeeff;
    
        // define multipliers for contacts in each location
        impact_intv_tmp = (beta[2] + upswing_rdeff_local) * covariates_local[3];
        
        // expand upswing time effect
        impact_intv_tmp .*= rep_matrix( to_vector( upswing_timeeff_reduced[ upswing_timeeff_map_local ]), A );

        // add other coeff*predictors
        impact_intv_tmp += covariates_local[1];
        impact_intv_tmp += ( beta[1] + dip_rdeff_local ) * covariates_local[2];
        for(t in 1:N2){
          impact_intv[t] = exp(rep_matrix(impact_intv_tmp[t,],A) .* chi);
        }
  ...
  matrix country_EeffcasesByAge(// parameters
        ...
        )
    {
        ...
        for (t in (N0+1):N2)
        {
            ...
            if(r_in(t, wkend_idx_local) == 1){
              lambdaByAge[t] = tmp_row_vector_A * (cntct_weekends_mean_local .* impact_intv[t]);
            }else{
              lambdaByAge[t] = tmp_row_vector_A * (cntct_weekdays_mean_local .* impact_intv[t]);
            }
        ...
        )
    {
        ...
        for (t in full_flows_Monday_idx_local:(full_flows_Monday_idx_local+n_days-1))
        {
            ...
            if(r_in(t, wkend_idx_local) == 1)
            {
                tmp_flow = rep_matrix(tmp_row_vector_A', A) .* cntct_weekends_mean_local .* impact_intv[t];
            }
            else
            {
                tmp_flow = rep_matrix(tmp_row_vector_A', A) .* cntct_weekdays_mean_local .* impact_intv[t];
            }
