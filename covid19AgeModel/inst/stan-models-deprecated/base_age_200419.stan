...
transformed parameters {
  ...
  for (m in 1:M)
  {
    ...
    for ( t in 1:N0 )
    {
      Prec_casesByAge[t,:] = E_casesByAge[m][t,:] * cntct_mean[m];
    ...
    for (t in (N0+1):N2)
    {
      ...
      E_casesByAge[m][t,:] = rev_serial_interval[max(1,(SI_CUT-t+2)):SI_CUT] * Prec_casesByAge[max(1,(t-SI_CUT)):(t-1),:];
      Prec_casesByAge[t,:] = E_casesByAge[m][t,:] * cntct_mean[m];
      Prec_casesByAge[t,:] *= cntct_intv[t,m];
    }
...
model {
  ...
  kappa ~ normal(0,0.5);
  R0 ~ normal(mean_R0, kappa); // citation: https://academic.oup.com/jtm/article/27/2/taaa021/5735319
