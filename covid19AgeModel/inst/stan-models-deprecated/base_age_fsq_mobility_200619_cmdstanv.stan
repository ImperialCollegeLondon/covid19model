functions {
      ...
      )
  {
    ...
    impact_intv = rep_matrix( append_col( log_reltransmissibility_age, 1.), N2);
...
data {
  ...
  row_vector<lower=0>[A] ifr_age_mean; // mean for probability of death in age band a
...
parameters {
  ...
  row_vector[A] log_ifr_age_inc; // increment of the probability of death from age a-1 to age a. ifr_age_inc[1] is the baseline
  real<lower=0> sd_ifr_age_inc;
  row_vector[A-1] log_relsusceptibility_age;
  real<lower=0> sd_log_relsusceptibility_age;
  row_vector[A-1] log_reltransmissibility_age;
  real<lower=0> sd_log_reltransmissibility_age;
}

transformed parameters {
    row_vector<lower=0,upper=1>[A] ifr_age = ifr_age_mean .* inv_logit( cumulative_sum( exp(log_ifr_age_inc) ) );
}
...
model {
  ...
  log_ifr_age_inc ~ normal(0, sd_ifr_age_inc);
  sd_ifr_age_inc ~ exponential(10);
  ...
  log_reltransmissibility_age ~ normal(0, sd_log_reltransmissibility_age);
...
generated quantities {
  ...
  for( m in 1:M )
  {
    ...
    // generate R_ta
    {
        ...
        RtByAge[m] = prop_susceptibleByAge .* rep_matrix( append_col(exp(log_relsusceptibility_age),1.), N2);
        for(t in 1:N2)
        {
