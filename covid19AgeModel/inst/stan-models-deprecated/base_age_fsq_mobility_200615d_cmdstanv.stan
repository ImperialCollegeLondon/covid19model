functions {
      ...
      )
  {
    ...
    impact_intv = rep_matrix( log_reltransmissibility_age, N2);
...
data {
  ...
  int<lower=1> A_REF; // reference for susceptibility and transmissibility
  ...
  real mean_log_relsusceptibility_age_reduced[A_PARS-1]; // log normal hyper-parameters for susceptibility OR
...
transformed parameters {
  row_vector[A] log_relsusceptibility_age = append_col(append_col(log_relsusceptibility_age_reduced[ age_band_map[1:(A_REF-1)] ], 0.), log_relsusceptibility_age_reduced[ age_band_map[(A_REF+1):A] ]);
  row_vector[A] log_reltransmissibility_age = append_col(append_col(log_reltransmissibility_age_reduced[ age_band_map[1:(A_REF-1)] ], 0.), log_reltransmissibility_age_reduced[ age_band_map[(A_REF+1):A] ]);
}
...
model {
  ...
  log_relsusceptibility_age_reduced ~ normal(mean_log_relsusceptibility_age_reduced, sd_log_relsusceptibility_age);
  sd_log_relsusceptibility_age ~ exponential(15);   // so the prior mean standard dev is 0.15
