...
data {
  ...
  real hyperpara_ifr_age[2,2]; // polynomial hyper-parameters for probability of death in age band a - first column is mean and second sd
...
transformed parameters{
  matrix<lower=0,upper=1>[M,A] ifr_age;
...
model {
  ...
  alpha ~ normal(hyperpara_ifr_age[:,1], hyperpara_ifr_age[:,2]);
