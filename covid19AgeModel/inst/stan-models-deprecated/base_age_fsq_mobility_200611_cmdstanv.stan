...
data {
  ...
  real hyperpara_ifr_age[2,2]; // Beta hyper-parameters for probability of death in age band a
  ...
  real age_midpoint[A]; // age continuous to to fit spline
}

transformed data {
  row_vector[A] log_age_midpoint = to_row_vector(log(age_midpoint)); 
...
parameters {
  ...
  real alpha[2]; // hyperparameters for polynomial form of ifr by age
  real alphaM[M]; // ifr baseline of states exchangeable
  real<lower=0> sigma_alphaM; // common variance of ifr by age random effect by state
}

transformed parameters{
  matrix<lower=0,upper=1>[M,A] ifr_age;// probability of death for age band a
  
  for(m in 1:M){
     ifr_age[m,:] = inv_logit(alpha[1] + alphaM[m] + alpha[2]*log_age_midpoint);
  }
}  
...
model {
  ...
  alpha ~ normal(hyperpara_ifr_age[:,1], 1);
  sigma_alphaM ~ cauchy(0,1);
  alphaM ~ normal(0,sigma_alphaM);
  ...
  for( m in 1:M )
  {
    ...
    rho0[m] = R0[m] / avg_cntct[m];
        ...
        for(t in 1:N2)
        {
            ...
            {
                RtByAge[m][t,:] = ( cntct_weekends_mean[m] * ones_vector_A )' .* impact_intv[t,:];
            }
            else
            {
                RtByAge[m][t,:] = ( cntct_weekdays_mean[m] * ones_vector_A )' .* impact_intv[t,:];
            }
        }
