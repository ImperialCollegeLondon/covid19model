...
transformed parameters {
  matrix[A,A] K = cov_exp_quad(age_midpoint, alpha, rho) + diag_matrix(rep_vector(1e-9, A));
  matrix[A, A] L_K = cholesky_decompose(K);
  row_vector<lower=0>[A] beta_age = to_row_vector(exp(a0 + L_K * eta));
}
  ...
  for( m in 1:M )
  {
    ...
    for(t in 1:N2){
      if(r_in(t, wkend_idx[,m]) == 1){
        RtByAge[m][t,:] = impact_intv[t,] * cntct_weekends_mean[m];
      } else{
        RtByAge[m][t,:] = impact_intv[t,] * cntct_weekdays_mean[m];
      }
      // eta[m][t,:] = (ones_row_vector_A*impact_intv[t])/rho0[m];
    }
