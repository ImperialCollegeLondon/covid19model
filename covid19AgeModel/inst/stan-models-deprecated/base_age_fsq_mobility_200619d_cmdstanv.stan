...
parameters {
  ...
  row_vector[A_PARS] logit_ifr_age_mult;
...
transformed parameters {
    row_vector<lower=0,upper=1>[A] ifr_age = ifr_age_mean .* ( ( 2 * inv_logit( logit_ifr_age_mult ) )[ age_band_map ] );
}
...
model {
  ...
  logit_ifr_age_mult[1] ~ normal(0, 0.002235509);   // sd <- 0.05; acor1 <- 0.999; sd_noi <- sqrt( (1-acor1*acor1)*(sd*sd) )
  logit_ifr_age_mult[2:A_PARS] ~ normal(0.999*logit_ifr_age_mult[1:(A_PARS-1)], 0.002235509);
