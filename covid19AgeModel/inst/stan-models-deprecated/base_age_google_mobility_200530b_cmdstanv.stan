functions {
                              ...
                              )
  {
    ...
    matrix[N2,A] E_casesByAge = country_EcasesByAge(R0_local, e_cases_N0_local, beta_res_wkend,
                              beta_res_wkday, beta_nonres_wkday, beta_age_local, N0, N2, A, SI_CUT, wkend_idx_local,
...
data {
  ...
  int n_age_continuous; // length of age in continuous scale
  real age_continuous[n_age_continuous]; // age continuous to to fit spline
  int num_knots;
  vector[num_knots] knots; // some age points
  matrix[A, n_age_continuous] get_age_discrete; // map continuous age to discrete age
  int spline_degree;  // the degree of spline (is equal to order - 1)
  matrix[num_knots + spline_degree - 1, n_age_continuous] B;  // matrix of B-splines
}
...
parameters {
  ...
  // splines parameters
  row_vector<lower=0>[num_basis] a_raw; // multiplier on basis functions
  real<lower=0> a0;  // intercept
  real<lower=0> aM[M]; // random effects
  real<lower=0> sigma;
  real<lower=0> rho;
}
...
transformed parameters {
  ...
  matrix[n_age_continuous, M] beta_age_cont;
  matrix[M, A] beta_age;
   a[1] = a_raw[1];
  for (i in 2:num_basis)
    a[i] = a[i-1] + a_raw[i]*rho;
    for(m in 1:M){
        beta_age_cont[,m] = rep_vector(a0 + aM[m], n_age_continuous) + to_vector(a*B);
        beta_age[m,] = to_row_vector(get_age_discrete * beta_age_cont[,m]);
    }
}
...
generated quantities {
  ...
  for( m in 1:M )
  {
    ...
    // define prop susc in local scope so it s not an output
    {
      ...
      for(age in 1:A){
        prop_susceptibleByAge[1,age] = 0;
        prop_susceptibleByAge[2:N2,age] = cumulative_sum( E_casesByAge[m][1:(N2-1),age] ) / popByAge_abs[m,age];
        for( t in 1:N2)
        {
          if( prop_susceptibleByAge[t,age]>1 )
            {
                prop_susceptibleByAge[t,age]= 1.;
            }
        }
      }
      prop_susceptibleByAge = rep_matrix(1.0, N2, A) - prop_susceptibleByAge;
      RtByAge[m] = prop_susceptibleByAge;
      for(t in 1:N2){
        RtByAge[m][t,:] = (cntct_weekends_mean[m] * (RtByAge[m][t,:])')' * impact_intv[2][t];
        RtByAge[m][t,:] += (cntct_weekdays_mean[m] * (RtByAge[m][t,:])')' * impact_intv[1][t];
       }
     }
