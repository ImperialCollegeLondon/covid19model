functions {
                              ...
                              )
  {
    ...
    E_deathsByAge .*= rep_matrix(exp(log_ifr_age), N2);
...
parameters {
  ...
  real upswing_rnde[M];
  real sd_upswing_rnde;
  real<lower=0> phi; // overdispersion parameter for likelihood model
  real<lower=0> ifr_noise[M];
  row_vector<upper=0>[A] log_ifr_age; // probability of death for age band a
...
model {
  ...
  log_ifr_age ~ normal(hyperpara_ifr_age_lnmu, hyperpara_ifr_age_lnsd);
