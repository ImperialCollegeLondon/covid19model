...
data {
  ...
  row_vector<lower=0>[A] inv_sqrt_1_to_A; // mean for probability of death in age band a
...
transformed data {
  ...
  row_vector[A] sd_corr = inv_sqrt_1_to_A;
...
parameters {
  ...
  row_vector[A] logit_ifr_age_mult;
...
transformed parameters {
    row_vector<lower=0,upper=1>[A] ifr_age = ifr_age_mean .* ( 2 * inv_logit( logit_ifr_age_mult ) );
}
...
model {
  ...
  // sd <- 0.05       std dev of each AR1 element
  // acor1 <- 0.999   lag1 autocorrelation
  // sd_noi <- sqrt( (1-acor1*acor1)*(sd*sd) )
  logit_ifr_age_mult[1] ~ normal(0, 0.002235509);
  logit_ifr_age_mult[2:A] ~ normal(0.999*logit_ifr_age_mult[1:(A-1)], 0.002235509);
