                          ...
                          )
  {
    ...
    for(m_slice in 1:M_slice) {
      ...
      if(map_country[m,1] == 1){
        ...
        lpmf += neg_binomial_2_lpmf(deaths_slice[m, epidemicStart[m]:(dataByAgestart[index_country_slice]-1)] | E_deaths[epidemicStart[m]:(dataByAgestart[index_country_slice]-1)], phi );
          for(a in 1:A_AD[index_country_slice]){
            min_age_slice = map_age[a,1,index_country_slice];
            max_age_slice = map_age[a,2,index_country_slice];
            lpmf += neg_binomial_2_lpmf(deathsByAge[dataByAgestart[index_country_slice]:N[m], a, index_country_slice] | 
                                          sum(E_deathsByAge[dataByAgestart[index_country_slice]:N[m], min_age_slice:max_age_slice]), phi );
          }
      }
      if(map_country[m,1] == 0){
        lpmf += neg_binomial_2_lpmf(deaths_slice[m, epidemicStart[m]:N[m]]| E_deaths[epidemicStart[m]:N[m]], phi );
      }
   }
    return(lpmf);
  }
...
data {
  ...
  int map_age[A, 2, M_AD]; // map the age groups reported with 5 y age group -- the row with i > A2[M_AD] contain -1 and should be ignored
