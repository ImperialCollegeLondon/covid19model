functions {
   /* 
   * build B-splines - code from https://mc-stan.org/users/documentation/case-studies/splines_in_stan.html
   */
                              ...
                              )
  {
    ...
    row_vector[A] prop_susceptibleByAge = rep_row_vector( zero, A );
                              ...
                              )
  {
    ...
    matrix[N2,A] E_casesByAge = country_EcasesByAge(R0_local, e_cases_N0_local, beta_res_wkend,
                              ...
                              cntct_weekends_mean_local, cntct_weekdays_mean_local, rev_serial_interval, popByAge_abs_local, init_A);
...
data {
  ...
  int num_knots; // length of age in continuous scale
  real knots[num_knots]; // age continuous scale to fit spline
  matrix[A,num_knots] discrete_knots; // map continuous age to discrete age
  int spline_degree;  // the degree of spline (is equal to order - 1)
  vector a.ifr.by.age.prior;
}
...
transformed data {
  ...
  matrix[num_basis, num_knots] B;  // matrix of B-splines
  ...
  B[ind,:] = to_row_vector(build_b_spline(knots, to_array_1d(ext_knots), ind, spline_degree + 1));
  B[num_knots + spline_degree - 1, num_knots] = 1;
}
...
parameters {
  ...
  // splines parameters
  row_vector[num_basis] a_raw_betaage;
  real a0_betaage;  // intercept
  real<lower=0> sigma_betaage;
  real<lower=0> rho_betaage;
  row_vector[num_basis] a_raw_ifrage;
  real a0_ifrage;  // intercept
  real<lower=0> sigma_ifrage;
  real<lower=0> rho_ifrage;
}

transformed parameters {
  row_vector[num_basis] a_betaage; // spline coefficients
  vector[num_knots] beta_age_cont;
  row_vector[A] beta_age;
  row_vector[num_basis] a_ifrage; // spline coefficients
  vector[num_knots] ifr_age_cont;
  row_vector[A] ifr_age;
  
  a_betaage[1] = a_raw_betaage[1];
  for (i in 2:num_basis){
    a_betaage[i] = a_betaage[i-1] + a_raw_betaage[i]*rho;
    a_ifrage[i] = a_ifrage[i-1] + a_raw_ifrage[i]*rho;
  }
  beta_age_cont = a0_betaage*to_vector(knots) + to_vector(a_betaage*B);
  beta_age = to_row_vector(discrete_knots * beta_age_cont);
  ifr_age_cont = a0_ifrage*to_vector(knots) + to_vector(a_ifrage*B);
  ifr_age = to_row_vector(discrete_knots * ifr_age_cont);
}
...
model {
  ...
  a_raw_betaage ~ normal(0, 1);
  a0_betaage ~ normal(0, 1);
  rho_betaage ~ normal(0, 1);
  sigma_betaage ~ normal(0, 1);
  a_raw_ifrage ~ normal(a.ifr.by.age.prior,1);
  a0_ifrage ~ normal(0, 1);
  rho_ifrage ~ normal(0, 1);
  sigma_ifrage ~ normal(0, 1);
...
generated quantities {
  ...
  matrix[N2,A] prop_susceptibleByAge[M];
  ...
  for( m in 1:M )
  {
    ...
    RtByAge[m] .*= rep_matrix( impact_intv[1], A);
    tmp = rep_matrix( ones_row_vector_A*cntct_weekdays_mean[m] , N2);
    tmp .*= rep_matrix( impact_intv[2], A);
    RtByAge[m] += tmp;
    ...
    for(age in 1:A){
      prop_susceptibleByAge[m][:,age] = rep_vector(1.0, N2) - (cumulative_sum(E_casesByAge[m][:,age]) / popByAge_abs[m,age]);
    }
   }
}
