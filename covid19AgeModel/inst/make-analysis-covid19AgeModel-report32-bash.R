...
{	
	...
	args <- data.table(			
			...
			seed= 42,
			chain= 1,
			job_tag= '37states_tau10_report32',			
			countries = countries,	
			cmdstan = 1L,
			multiplier_cntct_school_closure = 1.0,
			with_forecasts = 1,
			forecast_with_schools_reopened = 1, # if true, school re-opening date is set to 2020-08-24. if false, it is set to the first day of forecast (i.e., without death observation)
			ifr_by_age_prior = "BetaBinomial"				
			)
}
## Report 32 sensitivity analysis, assume 50% lower contacts during school period from/to 0-14
if(0)
{	
	countries <-  "AL,AZ,CA,CO,CT,DC,DE,FL,GA,IA,ID,IL,IN,KY,LA,MA,MD,MI,MO,MS,NC,NH,NJ,NM,NV,NYC,OK,OR,PA,RI,SC,TN,TX,UT,VA,WA,WI"
	n_countries <- length(unlist(strsplit(countries,',')))
	...
	args <- data.table(			
			...
			seed= 42,
			chain= 1,
			job_tag= '37states_tau05_report32',			
			countries = countries,	
			cmdstan = 1L,
...
## Report 32 sensitivity analysis, assume 1.5 times higher contacts during school period from/to 0-14
if(0)
{	
	countries <-  "AL,AZ,CA,CO,CT,DC,DE,FL,GA,IA,ID,IL,IN,KY,LA,MA,MD,MI,MO,MS,NC,NH,NJ,NM,NV,NYC,OK,OR,PA,RI,SC,TN,TX,UT,VA,WA,WI"
	n_countries <- length(unlist(strsplit(countries,',')))
	...
	args <- data.table(			
			...
			seed= 42,
			chain= 1,
			job_tag= '37states_tau15_report32',			
			countries = countries,	
			cmdstan = 1L,
			multiplier_cntct_school_closure = 1.5,
			with_forecasts = 0,
			forecast_with_schools_reopened = 0, 
			ifr_by_age_prior = "BetaBinomial"				
	)
}
## Report 32 sensitivity analysis, assume 2 times higher contacts during school period from/to 0-14
if(0)
{	
	countries <-  "AL,AZ,CA,CO,CT,DC,DE,FL,GA,IA,ID,IL,IN,KY,LA,MA,MD,MI,MO,MS,NC,NH,NJ,NM,NV,NYC,OK,OR,PA,RI,SC,TN,TX,UT,VA,WA,WI"
	n_countries <- length(unlist(strsplit(countries,',')))
	...
	args <- data.table(			
			...
			seed= 42,
			chain= 1,
			job_tag= '37states_tau20_report32',			
			countries = countries,	
			cmdstan = 1L,
			multiplier_cntct_school_closure = 2.0,
			with_forecasts = 0,
			forecast_with_schools_reopened = 0, 
			ifr_by_age_prior = "BetaBinomial"				
	)
}
## Report 32 sensitivity analysis, assume mobility trends extrapolate to 0-14
if(0)
{	
	countries <-  "AL,AZ,CA,CO,CT,DC,DE,FL,GA,IA,ID,IL,IN,KY,LA,MA,MD,MI,MO,MS,NC,NH,NJ,NM,NV,NYC,OK,OR,PA,RI,SC,TN,TX,UT,VA,WA,WI"
	n_countries <- length(unlist(strsplit(countries,',')))
	...
	args <- data.table(			
			...
			script_generate_quantities_file = "scripts/generate-quantities.r",
			script_rmd_file = "scripts/post-processing-make-report.Rmd",
			stanModelFile= 'base_age_fsq_mobility_200730j2_cmdstanv',
			hmc_stepsize= 0.02,
			hmc_num_samples= 1500,
			hmc_num_warmup= 1000,
			seed= 42,
...
## Report 32 assume Levin et al prior
if(0)
{	
  countries <-  "AL,AZ,CA,CO,CT,DC,DE,FL,GA,IA,ID,IL,IN,KY,LA,MA,MD,MI,MO,MS,NC,NH,NJ,NM,NV,NYC,OK,OR,PA,RI,SC,TN,TX,UT,VA,WA,WI"
  n_countries <- length(unlist(strsplit(countries,',')))
