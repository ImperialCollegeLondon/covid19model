data {
  ...
  int epidemicStart[M];
  int deaths_by_age[N2, A, M]; // reported deaths by age -- the rows with i > N contain -1 and should be ignored
  ...
  matrix[A,A] cntct_weekends_mean[M]; // mean of prior contact rates between age groups on weekendss
...
parameters {
  ...
  real index_contact;
  //	contact matrix priors
  matrix<lower=0>[A,A] cntct_weekdays[M]; 
  matrix<lower=0>[A,A] cntct_weekends[M]; 
  real sigma_we; // sd cntct_weekdays
  real sigma_wd; // sd cntct_weekends
}
...
transformed parameters {
  ...
  // variance of contact matrix
  row_vector<lower=0>[A] sigma2_wd_vector = rep_row_vector(sigma_wd^2,A);
  row_vector<lower=0>[A] sigma2_we_vector = rep_row_vector(sigma_we^2,A);
    ...
    for (m in 1:M)
  {
    for (t in (N0+1):N2)
    {
      ...
      E_casesByAge[m][t,:] = tmp_row_vector_A * ( impact_intv_res[t,m] * cntct_weekends[m] + impact_intv_nonres[t,m] * cntct_weekdays[m] );
    }
  }
...
model {
  ...
  sigma_wd ~ normal(0,0.5); 
  sigma_we ~ normal(0,0.5); 

  for(m in 1:M){
    for(a in 1:A){
      // prior 
      // log normal with mean cntct_weekends_mean and sd sigma
      cntct_weekdays[m][a,:] ~ lognormal(2*log(cntct_weekdays_mean[m][a,:]) - log(sigma2_wd_vector + square(cntct_weekdays_mean[m][a,:]))/2, sqrt(-2*log(cntct_weekdays_mean[m][a,:]) + log(sigma2_wd_vector + square(cntct_weekdays_mean[m][a,:]))));
      cntct_weekends[m][a,:] ~ lognormal(2*log(cntct_weekends_mean[m][a,:]) - log(sigma2_we_vector + square(cntct_weekends_mean[m][a,:]))/2, sqrt(-2*log(cntct_weekends_mean[m][a,:]) + log(sigma2_we_vector + square(cntct_weekends_mean[m][a,:]))));

      // likelihood
      deaths_by_age[epidemicStart[m]:N[m], a, m] ~ neg_binomial_2(E_deathsByAge[m][epidemicStart[m]:N[m], a], phi); 
    }
  }
}
