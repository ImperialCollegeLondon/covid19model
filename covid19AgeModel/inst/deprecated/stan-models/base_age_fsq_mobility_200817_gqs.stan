functions {
        ...
        )
    {
        ...
        impact_intv = (beta[3] + upswing_rdeff_local) * covariates_local[3];
        ...
        impact_intv += beta[2] * covariates_local[2];
    ...
    matrix country_EcasesByAge(// parameters
        ...
        int[] AGE_CHILD,
        int[] AGE_PARENT,
        real max_young_contact,
        real prop_min_child_parent_contact,
        real prop_min_parent_child_contact,
        int[] wkend_idx_local,
        ...
        )
    {
        ...
        for (t in (N0+1):N2)
        {
            ...
            matrix[A,A] cntct_weekends_mean_local_sens = cntct_weekends_mean_local;
            matrix[A,A] cntct_weekdays_mean_local_sens = cntct_weekdays_mean_local;

            // upper bound on contacts between young
            if(t > DIP_TIME_local){
              for(a in AGE_YOUNG){
                for(b in AGE_YOUNG){
                  {
                    // weekdays
                    if( cntct_weekdays_mean_local[a, b] * impact_intv[t,a] * impact_intv[t,b] < max_young_contact){
                      cntct_weekdays_mean_local_sens[a, b] = cntct_weekdays_mean_local[a, b];
                    }
                    else{
                      cntct_weekdays_mean_local_sens[a, b] = max_young_contact / (impact_intv[t,a] * impact_intv[t,b]);
                    } 
                    
                    // weekends
                    if( cntct_weekends_mean_local[a, b] * impact_intv[t,a] * impact_intv[t,b] < max_young_contact){
                      cntct_weekends_mean_local_sens[a, b] = cntct_weekends_mean_local[a, b];
                    }
                    else{
                      cntct_weekends_mean_local_sens[a, b] = max_young_contact / (impact_intv[t,a] * impact_intv[t,b]);
                    }  
                  }
                }
              }
            ...
            //  lower bounds on child and parent contacts 
            for(a in AGE_CHILD){
                for(b in AGE_PARENT){
                    // weekdays
                    // child-parent
                    if( cntct_weekdays_mean_local[a, b] * impact_intv[t,a] * impact_intv[t,b] < cntct_weekdays_mean_local[a, b] * prop_min_child_parent_contact){
                      cntct_weekdays_mean_local_sens[a, b] = (cntct_weekdays_mean_local[a, b] * prop_min_child_parent_contact) / (impact_intv[t,a] * impact_intv[t,b]);
                    }
                    else{
                      cntct_weekdays_mean_local_sens[a, b] =  cntct_weekdays_mean_local[a, b] ;
                    } 
                    // parent-child
                    if( cntct_weekdays_mean_local[b,a] * impact_intv[t,a] * impact_intv[t,b] < cntct_weekdays_mean_local[b,a] * prop_min_parent_child_contact){
                      cntct_weekdays_mean_local_sens[b,a] = (cntct_weekdays_mean_local[b,a] * prop_min_parent_child_contact) / (impact_intv[t,a] * impact_intv[t,b]);
                    }
                    else{
                      cntct_weekdays_mean_local_sens[b,a] =  cntct_weekdays_mean_local[b,a] ;
                    } 
                    
                    // weekends
                    // child-parent
                    if( cntct_weekends_mean_local[a, b] * impact_intv[t,a] * impact_intv[t,b] < cntct_weekends_mean_local[a, b] * prop_min_child_parent_contact){
                      cntct_weekends_mean_local_sens[a, b] = (cntct_weekends_mean_local[a, b] * prop_min_child_parent_contact) / (impact_intv[t,a] * impact_intv[t,b]);
                    }
                    else{
                      cntct_weekends_mean_local_sens[a, b] =  cntct_weekends_mean_local[a, b] ;
                    } 
                    // parent-child
                    if( cntct_weekends_mean_local[b,a] * impact_intv[t,a] * impact_intv[t,b] < cntct_weekends_mean_local[b,a] * prop_min_parent_child_contact){
                      cntct_weekends_mean_local_sens[b,a] = (cntct_weekends_mean_local[b,a] * prop_min_parent_child_contact) / (impact_intv[t,a] * impact_intv[t,b]);
                    }
                    else{
                      cntct_weekends_mean_local_sens[b,a] =  cntct_weekends_mean_local[b,a] ;
                    } 
            ...
            {
                E_casesByAge[t] = tmp_row_vector_A * cntct_weekends_mean_local_sens;
            }
            else
            {
                E_casesByAge[t] = tmp_row_vector_A * cntct_weekdays_mean_local_sens;
            }
        ...
        )
    {
        ...
        for(t in 1:N2)
        {
          ...
          // upper bound on contacts between young
            ...
            if(r_in(t, wkend_idx_local) == 1)
            {
                RtByAge[t,:] = ( (impact_intv[t,:] .* RtByAge[t,:]) * (cntct_weekends_mean_local_sens') ) .* impact_intv[t,:];
            }
            else
            {
                RtByAge[t,:] = ( (impact_intv[t,:] .* RtByAge[t,:]) * (cntct_weekdays_mean_local_sens') ) .* impact_intv[t,:];
            }
        }
        return(RtByAge);
    }
        ...
        )
    {
        ...
        for (t in (N0+1):N2)
        {
          ...
          // upper bound on contacts between young
            ...
            if(r_in(t, wkend_idx_local) == 1){
              lambdaByAge[t] = tmp_row_vector_A * cntct_weekends_mean_local_sens;
            }else{
              lambdaByAge[t] = tmp_row_vector_A * cntct_weekdays_mean_local_sens;
            }
        ...
        )
    {
        ...
        for (t in full_flows_Monday_idx_local:(full_flows_Monday_idx_local+n_days-1))
        {
          ...
          // upper bound on contacts between young
            ...
            if(r_in(t, wkend_idx_local) == 1)
            {
                tmp_flow = rep_matrix(tmp_row_vector_A', A) .* cntct_weekends_mean_local_sens;
            }
            else
            {
                tmp_flow = rep_matrix(tmp_row_vector_A', A) .* cntct_weekdays_mean_local_sens;
            }
        ...
        for (week in 1:N_WEEKS_FULL_FLOWS_local)
        {
            full_flows[week] =
                country_EflowsByHighDimAge_AggregateDays(
                    ...
                    AGE_PARENT,
                    max_young_contact,
                    prop_min_child_parent_contact,
                    prop_min_parent_child_contact,
                    wkend_idx_local,
...
data {
  ...
  int<lower=1> DIP_TIME[M]; // time when mobility trend starts to decrease
  ...
  // Contact
  int A_YOUNG; // number of age band with young
  int<lower=1,upper=A> AGE_YOUNG[A_YOUNG]; // age bands with young 
  int A_CHILD; // number of age band for child
  int<lower=1,upper=A> AGE_CHILD[A_CHILD]; // age bands with chilf 
  int A_PARENT; // number of age band with parent
  int<lower=1,upper=A> AGE_PARENT[A_PARENT]; // age bands with parent 
  real max_young_contact;
  real prop_min_child_parent_contact;
  real prop_min_parent_child_contact;
  //	priors
