functions {
        ...
        )
    {
        ...
        ifr_time = append_row(
            rep_vector(1., 3),
            exp( cumulative_sum( rep_vector( log_ifr_overall_upswing_effect, N_IMP-3 ) ) )
            );
                    
        // apply ifr by age and ifr by time to deaths
        E_deathsByAge .*= rep_matrix( ifr_age, N2);
        E_deathsByAge .*= rep_matrix( ifr_time[ upswing_timeeff_map_local ], A);
...
model {
  ...
  target += exponential_lpdf( -log_ifr_overall_upswing_effect | 33); // exp( -.03 * 0:10 ) = 1.0000000 ... 0.7408182
