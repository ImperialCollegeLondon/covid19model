functions {
        ...
        )
    {
        ...
        // contact matrix for counterfactual scenarios in which schools remain close
        matrix[A,A] cntct_elementary_school_remains_closed_weekdays_local = cntct_school_closure_weekdays_local;
        matrix[A,A] cntct_elementary_school_remains_closed_weekends_local = cntct_school_closure_weekends_local;
        
        // contact matrix after school reopening adjusted for NPIs
        matrix[A,A] cntct_elementary_elt_school_reopening_weekends_local = cntct_elementary_school_reopening_weekends_local;
        matrix[A,A] cntct_elementary_elt_school_reopening_weekdays_local = cntct_elementary_school_reopening_weekdays_local;
        ...
        // after school reopening, adjust the matrix for NPIs
        cntct_elementary_elt_school_reopening_weekends_local[1:A_CHILD, 1:A_CHILD] = elt_school_intv_effect * cntct_elementary_school_reopening_weekends_local[1:A_CHILD, 1:A_CHILD];
        cntct_elementary_elt_school_reopening_weekends_local[1:A_CHILD, AGE_ADULT] = min([1, 2*elt_school_intv_effect]) * cntct_elementary_school_reopening_weekends_local[1:A_CHILD, AGE_ADULT];
        cntct_elementary_elt_school_reopening_weekends_local[AGE_ADULT, 1:A_CHILD] = min([1, 2*elt_school_intv_effect]) * cntct_elementary_school_reopening_weekends_local[AGE_ADULT, 1:A_CHILD];
        
        cntct_elementary_elt_school_reopening_weekdays_local[1:A_CHILD, 1:A_CHILD] = elt_school_intv_effect * cntct_elementary_school_reopening_weekdays_local[1:A_CHILD, 1:A_CHILD];
        cntct_elementary_elt_school_reopening_weekdays_local[1:A_CHILD, AGE_ADULT] = min([1, 2*elt_school_intv_effect]) * cntct_elementary_school_reopening_weekdays_local[1:A_CHILD, AGE_ADULT];
        cntct_elementary_elt_school_reopening_weekdays_local[AGE_ADULT, 1:A_CHILD] = min([1, 2*elt_school_intv_effect]) * cntct_elementary_school_reopening_weekdays_local[AGE_ADULT, 1:A_CHILD];
        
        
        // for the counterfactual scenarios, adjust the forecast matrix so that it is consistent with the inferred contact matrix
        for(a in 1:A_CHILD)
        {
            // update children in rows and all other in cols
            for(aprime in 1:A)
            {
                cntct_elementary_school_remains_closed_weekends_local[a, aprime] =
                    min([
                        cntct_elementary_elt_school_reopening_weekends_local[a, aprime],
                        cntct_school_closure_weekends_local[a, aprime]
                        ]);
                cntct_elementary_school_remains_closed_weekdays_local[a, aprime] =
                    min([
                        cntct_elementary_elt_school_reopening_weekdays_local[a, aprime],
                        cntct_school_closure_weekdays_local[a, aprime]
                        ]);
            }
            // update children in cols and adults in rows
            for(aprime in (A_CHILD+1):A)
            {
                cntct_elementary_school_remains_closed_weekends_local[aprime, a] =
                    min([
                        cntct_elementary_elt_school_reopening_weekends_local[aprime, a],
                        cntct_school_closure_weekends_local[aprime, a]
                        ]);
                cntct_elementary_school_remains_closed_weekdays_local[aprime, a] =
                    min([
                        cntct_elementary_elt_school_reopening_weekdays_local[aprime, a],
                        cntct_school_closure_weekdays_local[aprime, a]
                        ]);
            }
        }
        ...
        for (t in (N0+1):N2)
        {
            ...
            if(r_in(t, wkend_idx_local) == 1)
            {
                ...
                else if(SCHOOL_STATUS_local[t] == 0 && t>=elementary_school_reopening_idx_local)
                {
                    E_casesByAge[t] =
                        append_col(
                            tmp_row_vector_A_no_impact_intv * cntct_elementary_elt_school_reopening_weekends_local[,1:A_CHILD],
                            tmp_row_vector_A_no_impact_intv[1:A_CHILD] * cntct_elementary_elt_school_reopening_weekends_local[1:A_CHILD,(A_CHILD+1):A] +
                                (tmp_row_vector_A[(A_CHILD+1):A] * cntct_elementary_elt_school_reopening_weekends_local[(A_CHILD+1):A,(A_CHILD+1):A]) .* impact_intv[t,(A_CHILD+1):A]
                            );
                ...
                }else if(SCHOOL_STATUS_local[t] == 1 && t>=elt_reopening_index_local)
                {
                    E_casesByAge[t] =
                        append_col(
                            tmp_row_vector_A_no_impact_intv * cntct_elementary_school_remains_closed_weekends_local[,1:A_CHILD],
                            tmp_row_vector_A_no_impact_intv[1:A_CHILD] * cntct_elementary_school_remains_closed_weekends_local[1:A_CHILD,(A_CHILD+1):A] +
                                (tmp_row_vector_A[(A_CHILD+1):A] * cntct_elementary_school_remains_closed_weekends_local[(A_CHILD+1):A,(A_CHILD+1):A]) .* impact_intv[t,(A_CHILD+1):A]
                            );
                }
                else
                {
            ...
            else
            {
                ...
                else if(SCHOOL_STATUS_local[t] == 0 && t>=elementary_school_reopening_idx_local)
                {
                    E_casesByAge[t] =
                        append_col(
                            tmp_row_vector_A_no_impact_intv * cntct_elementary_elt_school_reopening_weekdays_local[,1:A_CHILD],
                            tmp_row_vector_A_no_impact_intv[1:A_CHILD] * cntct_elementary_elt_school_reopening_weekdays_local[1:A_CHILD,(A_CHILD+1):A] +
                                (tmp_row_vector_A[(A_CHILD+1):A] * cntct_elementary_elt_school_reopening_weekdays_local[(A_CHILD+1):A,(A_CHILD+1):A]) .* impact_intv[t,(A_CHILD+1):A]
                            );
                }else if(SCHOOL_STATUS_local[t] == 1 && t>=elt_reopening_index_local)
                {
                    E_casesByAge[t] =
                        append_col(
                            tmp_row_vector_A_no_impact_intv * cntct_elementary_school_remains_closed_weekdays_local[,1:A_CHILD],
                            tmp_row_vector_A_no_impact_intv[1:A_CHILD] * cntct_elementary_school_remains_closed_weekdays_local[1:A_CHILD,(A_CHILD+1):A] +
                                (tmp_row_vector_A[(A_CHILD+1):A] * cntct_elementary_school_remains_closed_weekdays_local[(A_CHILD+1):A,(A_CHILD+1):A]) .* impact_intv[t,(A_CHILD+1):A]
                            );
                }
                else
                {
        ...
        )
    {
        ...
        for (t in full_flows_Monday_idx_local:(full_flows_Monday_idx_local+n_days-1))
        {
            ...
            if(r_in(t, wkend_idx_local) == 1)
            {
                ...
                {
                    tmp_flow = append_col( rep_matrix(tmp_row_vector_A_no_impact_intv', A_CHILD), rep_matrix(tmp_row_vector_A', A-A_CHILD) ) .* cntct_elementary_elt_school_reopening_weekends_local;
                    tmp_flow[1:A_CHILD,(A_CHILD+1):A] *= elt_school_intv_effect;
                }
                else if(SCHOOL_STATUS_local[t] == 1 && t>=elt_reopening_index_local)
                {
                  tmp_flow = append_col( rep_matrix(tmp_row_vector_A_no_impact_intv', A_CHILD), rep_matrix(tmp_row_vector_A', A-A_CHILD) ) .* cntct_elementary_school_remains_closed_weekends_local;
                }
                else
                {
            ...
            else
            {
                ...
                {
                    tmp_flow = append_col( rep_matrix(tmp_row_vector_A_no_impact_intv', A_CHILD), rep_matrix(tmp_row_vector_A', A-A_CHILD) ) .* cntct_elementary_elt_school_reopening_weekdays_local;
                }
                else if(SCHOOL_STATUS_local[t] == 1 && t>=elt_reopening_index_local)
                {
                    tmp_flow = append_col( rep_matrix(tmp_row_vector_A_no_impact_intv', A_CHILD), rep_matrix(tmp_row_vector_A', A-A_CHILD) ) .* cntct_elementary_school_remains_closed_weekdays_local;
                }
                else
                {
...
data {
  ...
  int<lower=1,upper=A> AGE_ADULT[A_ADULT]; // age bands with adult
...
{
    real rho0;
    real counterfactual_elt_school_intv_effect;
    ...
    // school effect, if conterfactual scenario set to the forecast value, either use elt_school_intv_effect
    if(counterfactual_school_effect >= 0){
      counterfactual_elt_school_intv_effect = counterfactual_school_effect;
    } else{
      counterfactual_elt_school_intv_effect = elt_school_intv_effect;
    }
    ...
    // generate expected cases by age + expected deaths by age
    {
        ...
        E_casesByAge =
            country_EcasesByAge(
                ...
                e_cases_N0[m],
                log_relsusceptibility_age,
                counterfactual_elt_school_intv_effect,
                impact_intv,
                N0,
