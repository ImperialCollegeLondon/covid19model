functions {
        ...
        )
    {
        ...
        for (t in (N0+1):N2)
        {
            ...
            row_vector[A] prop_susceptibleByAge = rep_row_vector(1.0, A) - inv_logit( logit_acquire_immunity) * (rep_row_vector(1.0, t-1) * E_casesByAge[1:(t-1),:] ./ popByAge_abs_local);
        ...
        )
    {
        ...
        for(a in 1:A)
        {
            ...
            prop_susceptibleByAge[2:N2,a] = inv_logit( logit_acquire_immunity) * cumulative_sum( E_casesByAge_local[1:(N2-1),a] ) / popByAge_abs_local[a];
            for( t in 1:N2)
            {
                          ...
                          )
  {
    ...
    for(m_slice in 1:M_slice) {
      ...
      // regularising prior
      //lpmf += gamma_lpdf( to_array_1d(Rt_byAge) | 1., 0.5);
...
model {
  ...
  sd_log_ifr_age_rnde_old ~ exponential(10);
  ...
  sd_upswing_rnde ~ exponential(40);
  logit_acquire_immunity ~ normal(0,0.5);
