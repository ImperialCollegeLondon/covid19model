functions {
    ...
    matrix country_EdeathsByAge(// parameters
        ...
        // data
        int N2,
        int rebound_date_time_idx_local,
        int A,
        ...
        real log_ifr_overall_upswing_effect
        )
    {
        real zero = 0.0;
        
        matrix[N2,A] E_deathsByAge = rep_matrix( zero, N2, A );
        matrix[N2,A] log_ifr_age;
        ...
        log_ifr_age = rep_matrix( log_ifr_age_base +
        ...
        log_ifr_age[(rebound_date_time_idx_local):N2] += rep_matrix( log_ifr_overall_upswing_effect , N2 - rebound_date_time_idx_local + 1, A);
        
        E_deathsByAge .*= exp( log_ifr_age );
...
parameters {
  ...
  real<upper=0> log_ifr_overall_upswing_effect;
...
model {
  ...
  target += exponential_lpdf( -log_ifr_overall_upswing_effect | 10);
  ...
  target += exponential_lpdf( sd_upswing_rnde | 10);
  target += normal_lpdf( upswing_rnde | 0, sd_upswing_rnde);
  target += exponential_lpdf( sd_upswing_timeeff_reduced | 10);
  target += normal_lpdf(upswing_timeeff_reduced[1] | 0, 0.025);
  target += normal_lpdf(upswing_timeeff_reduced[2:N_IMP] | upswing_timeeff_reduced[1:(N_IMP-1)], sd_upswing_timeeff_reduced);
  target += exponential_lpdf( hyper_timeeff_shift_mid1 | .1);
  target += exponential_lpdf( hyper_timeeff_shift_mid2 | .1);
  target += exponential_lpdf( hyper_timeeff_shift_old | .1);
  target += exponential_lpdf( timeeff_shift_mid1 | hyper_timeeff_shift_mid1);
  target += exponential_lpdf( timeeff_shift_mid2 | hyper_timeeff_shift_mid2);
  target += exponential_lpdf( timeeff_shift_old | hyper_timeeff_shift_old);
