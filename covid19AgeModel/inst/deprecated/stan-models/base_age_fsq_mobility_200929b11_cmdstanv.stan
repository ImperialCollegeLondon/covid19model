functions {
    ...
    matrix country_impact(
        real rndeff_before_rebound_local,
        real rndeff_after_rebound_local,
        row_vector TIME_IDX_BEFORE_REBOUND_local,
        ...
        )
    {
        ...
        // biweekly time effect
        impact_intv = rep_matrix( to_vector( upswing_timeeff_reduced[ upswing_timeeff_map_local ]), A );
        
        // age shift over time
        impact_intv += rep_matrix(timeeff_shift_age_local, N2);
        
        // add random effect before and after rebound
        impact_intv .*= rep_matrix( to_vector(rndeff_before_rebound_local * TIME_IDX_BEFORE_REBOUND_local + rndeff_after_rebound_local * (1 - TIME_IDX_BEFORE_REBOUND_local)), A);
        ...
        )
    {
        ...
        E_deathsByAge .*= rep_matrix(exp( log_ifr_age_base), N2);
    ...
    real countries_log_dens(int[,] deaths_slice,
        ...
        real[] e_cases_N0,
        real[] upswing_timeeff_reduced,
        vector rndeff_before_rebound,
        vector rndeff_after_rebound,
        matrix TIME_IDX_BEFORE_REBOUND,
        ...
        )
  {
    ...
    for(m_slice in 1:M_slice) {
      ...
      impact_intv =
        country_impact(
            rndeff_before_rebound[m],
            rndeff_after_rebound[m],
            TIME_IDX_BEFORE_REBOUND[m,],
...
data {
  ...
  int REBOUND_TIME_INDEX[M]; // time index of the first day of rebound period
  //	data
  real pop[M];
...
transformed data {
  ...
  matrix<lower=0,upper=1>[M,N2] TIME_IDX_BEFORE_REBOUND;
  ...
  for( m in 1:M )
  {
    ...
    TIME_IDX_BEFORE_REBOUND[m,] = append_col( rep_row_vector(1.0, REBOUND_TIME_INDEX[m] - 1), rep_row_vector(0, N2 - REBOUND_TIME_INDEX[m] + 1));
  }
}
...
parameters {
  ...
  vector[M] timeeff_shift_1;
  real<lower=0> sd_timeeff_shift_1;
  vector[M] timeeff_shift_2;
  real<lower=0> sd_timeeff_shift_2;
  vector[M] timeeff_shift_3;
  real<lower=0> sd_timeeff_shift_3;
  vector[M] timeeff_shift_4;
  real<lower=0> sd_timeeff_shift_4;
  vector[M] rndeff_before_rebound;
  real<lower=0> sd_rndeff_before_rebound;
  vector[M] rndeff_after_rebound;
  real<lower=0> sd_rndeff_after_rebound;
}
...
transformed parameters {
  ...
  matrix[M,A] timeeff_shift_age = append_col( append_col( append_col( append_col( rep_matrix(0., M, 3),  rep_matrix(timeeff_shift_1, 3)), 
                                  rep_matrix(timeeff_shift_2, 4)), rep_matrix(timeeff_shift_3, 3)), 
                                  rep_matrix(timeeff_shift_4, 5));
    
}

model {
  // priors
  e_cases_N0 ~ lognormal(4.5, 0.62); // qlnorm(c(0.01,0.5,0.99), 4.5, 0.62) = 21.27780  90.01713 380.82337
  phi ~ normal(0,5);
  kappa ~ normal(0,.5);
  ...
  rndeff_before_rebound ~ normal(0, sd_rndeff_before_rebound);
  sd_rndeff_before_rebound ~ exponential(10);
  rndeff_after_rebound ~ normal(0, sd_rndeff_after_rebound);
  sd_rndeff_after_rebound ~ exponential(10);
  ...
  timeeff_shift_1 ~ normal(0,sd_timeeff_shift_1);
  sd_timeeff_shift_1 ~ exponential(10);
  timeeff_shift_2 ~ normal(0,sd_timeeff_shift_2);
  sd_timeeff_shift_2 ~ exponential(10);
  timeeff_shift_3 ~ normal(0,sd_timeeff_shift_3);
  sd_timeeff_shift_3 ~ exponential(10);
  timeeff_shift_4 ~ normal(0,sd_timeeff_shift_4);
  sd_timeeff_shift_4 ~ exponential(10);
  ...
  target += reduce_sum(countries_log_dens, trans_deaths, 1,
      ...
      e_cases_N0,
      upswing_timeeff_reduced,
      rndeff_before_rebound,
      rndeff_after_rebound,
      TIME_IDX_BEFORE_REBOUND,
      timeeff_shift_age,
      log_relsusceptibility_age,
      phi,
      elt_school_intv_effect,
