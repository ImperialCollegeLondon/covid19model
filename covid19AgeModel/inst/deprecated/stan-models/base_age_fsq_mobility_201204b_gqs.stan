...
matrix country_contacts(// parameters
        ...
        for(t in 1:N2)
        {
            ...
            if(r_in(t, wkend_idx_local) == 1)
            {
                ...
                else if(SCHOOL_STATUS_local[t] == 0 && t>=elementary_school_reopening_idx_local)
                {
                    ...
                    RtByAge[t, 1:A_CHILD] *= impact_intv_children_effect;
                    RtByAge[t, (A_CHILD+1):A] .*= impact_intv[t,(A_CHILD+1):A];
                }
                // counterfactual scenario schools remain closed
                else if(SCHOOL_STATUS_local[t] == 1 && t>=elt_reopening_index_local)
                {
        ...
        for (t in (N0+1):N2)
        {
            ...
            if(r_in(t, wkend_idx_local) == 1)
            {
                ...
                else if(SCHOOL_STATUS_local[t] == 0 && t>=elementary_school_reopening_idx_local)
                {
                    ...
                    lambdaByAge[t, 1:A_CHILD] *= impact_intv_children_effect;
                    lambdaByAge[t, (A_CHILD+1):A] .*= impact_intv[t,(A_CHILD+1):A];
                }
                else if(SCHOOL_STATUS_local[t] == 1 && t>=elt_reopening_index_local)
                {
                    // for the counterfactual scenarios, adjust the contact matrix so that it is at most as large as the inferred contact matrix
    ...
    matrix country_EdeathsByAge(// parameters
        ...
        int timeidx_start_ifr_decay_local
        )
    {
        real zero;
        ...
        zero = 0.0;
        E_deathsByAge = rep_matrix( zero, N2, A );
        ...
        // calculate ifr by time
        // time eff 3 ends on calendar week 19, 2020-05-10
        // "linspaced_array" more effective but Stan v2.24 required
        ifr_time = append_row(
            rep_vector(1., timeidx_start_ifr_decay_local-1),
            exp(log_ifr_overall_upswing_effect * cumulative_sum( rep_vector(1 / (N2-timeidx_start_ifr_decay_local + 1.0) , N2-timeidx_start_ifr_decay_local +1 ) ) )
            );
                    
        // apply ifr by age and ifr by time to deaths
        E_deathsByAge .*= rep_matrix( ifr_age, N2);
        E_deathsByAge .*= rep_matrix( ifr_time, A);
        
        // avoid numerical catastrophe
        E_deathsByAge += 1e-15;
...
data {
  ...
  // ifr decay
  //
  real<lower=0> lambda_log_ifr_overall_upswing_effect;
  int<lower=1,upper=N2> timeidx_start_ifr_decay[M]; // time index for the start of ifr's decay
  //
  // mobility trends
  //  
...
parameters {
  ...
  real<upper=0> log_ifr_overall_upswing_effect; // total ifr decay at the end of the period
}
