data {
  ...
  real serial_interval[SI_CUT]; // fixed pre-calculated serial interval using empirical data from Neil in reverse order
...
transformed data{
  ...
  for( m in 1:M )
  {
    avg_cntct[m] = mean( ones_row_vector_A * cntct_mean[m] );
  }
}
...
transformed parameters {
  ...
  for (m in 1:M)
  {
    Prec_casesByAge[1:N0,init_A] = rep_vector( e_cases_N0[m], N0 );
    ...
    for (t in (N0+1):N2)
    {
      for ( a_prime in 1:A)
      {
        ...
        {
       // saturation_adj= ( pop[m] - sum( E_casesByAge[m][1:(t-1),a] ) ) / pop[m]; // TODO we need age specific pop counts
        Prec_casesByAge[t,a_prime] += serial_interval[t-s] * E_casesByAge[m][s,a_prime];
        }
      }
      
      for ( a in 1:A)
      {
        E_casesByAge[m][t,a] = sum(cntct_mean[m][:,a] .* to_vector(Prec_casesByAge[t,:]));
        E_casesByAge[m][t,a] *= rho0[m] * cntct_intv[t,m];
      }
    }
  }
...
generated quantities {
  ...
  for( m in 1:M )
  {
    Rt[m] = rep_matrix( ones_row_vector_A * cntct_mean[m], N2);
    Rt[m] .*= rep_matrix( cntct_intv[:,m], A);
    Rt[m] *= rho0[m];
  }
}