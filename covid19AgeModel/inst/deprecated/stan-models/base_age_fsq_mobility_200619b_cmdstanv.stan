functions {
      ...
      )
  {
    ...
    impact_intv = rep_matrix( append_col( log_reltransmissibility_age, 0.), N2);
...
transformed parameters {
    row_vector<lower=0,upper=1>[A] ifr_age = ifr_age_mean .* 2 * inv_logit( exp( sd_corr .* cumulative_sum( log_ifr_age_inc) ) );
}
