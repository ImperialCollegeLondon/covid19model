functions {
    ...
    real countries_log_dens(int[,] deaths_slice,
        ...
        int[] seroprevalence_surveys_n,
        int[,] seroprevalence_date_map,
        matrix[] seroprevalence_data,
        int[] seroprevalence_g18_surveys_n,
        int[,] seroprevalence_g18_date_map,
        int[,] seroprevalence_g18_index_map
        )
  {
    real lpmf = 0.0;
    ...
    vector[N2] E_cases;
    vector[N2] E_deaths;
    vector[N2] E_cum_antibody;
    vector[N2] E_cum_antibody_g18;
    matrix[N2, A] impact_intv;
    matrix[N2, A] E_casesByAge;
    matrix[N2, A] E_deathsByAge;
    matrix[N2, A] E_antibodyByAge;
    ...
    for(m_slice in 1:M_slice) {
      ...
      vector[seroprevalence_surveys_n[m]] E_seroprevalence;
      vector[seroprevalence_surveys_n[m]] E_seroprevalence_mean;
      vector[seroprevalence_surveys_n[m]] E_seroprevalence_sd;
      real lik1;
      real lik2;
      ...
      //
      // add
      // likelihood seroprevalence data this location
      // if state has seroprevalence survey estimates
      //
      if(seroprevalence_surveys_n[m] > 0)
      {
        
        E_antibodyByAge = country_EantibodyByAge(
          E_casesByAge,
          N2,
          A,
        ...
        E_cum_antibody = cumulative_sum( E_antibodyByAge * ones_vector_A);
        E_seroprevalence = E_cum_antibody[ seroprevalence_date_map[m,1:seroprevalence_surveys_n[m]] ] ./ pop[m];
        
        if(seroprevalence_g18_surveys_n[m] > 0)
        {
            E_cum_antibody_g18 =  cumulative_sum( E_antibodyByAge * append_row(to_vector([0, 0, 0, 0.4]), ones_vector_A[5:18]) );
            E_seroprevalence[ seroprevalence_g18_index_map[m,1:seroprevalence_g18_surveys_n[m]] ] =
                E_cum_antibody_g18[ seroprevalence_g18_date_map[m,1:seroprevalence_g18_surveys_n[m]] ] ./ pop[m];
        }
        //for( t in 1:seroprevalence_surveys_n[m])
        //{
            //if(E_seroprevalence[t] >= .99)
            //{
            //    E_seroprevalence[t] = .99;
            //}
        //}
        
        E_seroprevalence_mean = seroprevalence_data[m, 1:seroprevalence_surveys_n[m], 3] .* E_seroprevalence;
        E_seroprevalence_sd = sqrt( seroprevalence_data[m, 1:seroprevalence_surveys_n[m], 3] .* E_seroprevalence .* (1 - E_seroprevalence ) );
                
        // print("sd", E_seroprevalence_sd );
        // print("obs",seroprevalence_data[m, 1:seroprevalence_surveys_n[m], 2]);
        // print("mean",E_seroprevalence_mean);
        lpmf += (
            normal_lcdf(seroprevalence_data[m, 1:seroprevalence_surveys_n[m], 2] | E_seroprevalence_mean, E_seroprevalence_sd) -
            normal_lcdf(seroprevalence_data[m, 1:seroprevalence_surveys_n[m], 1] | E_seroprevalence_mean, E_seroprevalence_sd)
            );
        // print("lok1", normal_lcdf(seroprevalence_data[m, 1:seroprevalence_surveys_n[m], 2] | E_seroprevalence_mean, E_seroprevalence_sd);
        // print("lok2", normal_lcdf(seroprevalence_data[m, 1:seroprevalence_surveys_n[m], 1] | E_seroprevalence_mean, E_seroprevalence_sd);
      }
   }
    return(lpmf);
  }
...
data {
  ...
  //
  // seroprevalence data by age
  //
  int seroprevalence_surveys_n_max;
  int<lower=-1, upper=seroprevalence_surveys_n_max> seroprevalence_surveys_n[M]; // number of date indices per location, if -1 no seroprevalence estimate to the loc
  int seroprevalence_date_map[M, seroprevalence_surveys_n_max]; // map of date indices to time indices
  matrix[seroprevalence_surveys_n_max, 3] seroprevalence_data[M]; // likelihood parameters for observed cases with antibodies
  int seroprevalence_g18_surveys_n_max;
  int<lower=-1, upper=seroprevalence_g18_surveys_n_max> seroprevalence_g18_surveys_n[M];
  int seroprevalence_g18_date_map[M, seroprevalence_g18_surveys_n_max];
  int seroprevalence_g18_index_map[M, seroprevalence_g18_surveys_n_max];
  //
...
model {
  // priors
  e_cases_N0 ~ lognormal(4.85, 0.4); // qlnorm(c(0.01,0.5,0.99), 4.85, 0.4) = 50.37328 127.74039 323.93379
  phi ~ normal(0,5);
  beta ~ normal(0,1);
  R0 ~ lognormal(0.98, 0.2); // qlnorm(c(0.025,0.5,0.975), .98, 0.2) = 1.800397 2.664456 3.943201
  ...
  sd_dip_rnde ~ exponential(1.5);
  dip_rnde ~ normal(0, sd_dip_rnde);
  ...
  target += reduce_sum(countries_log_dens, trans_deaths, 1,
      ...
      seroprevalence_surveys_n,
      seroprevalence_date_map,
      seroprevalence_data,
      seroprevalence_g18_surveys_n,
      seroprevalence_g18_date_map,
      seroprevalence_g18_index_map
      );
}
