functions {
        ...
        )
    {
        ...
        // for the counterfactual scenarios, adjust the contact matrix so that it is consistent with the inferred contact matrix
        for(a in 1:A_CHILD)
        {
            for(aprime in 1:A_CHILD)
            {
        ...
        for (t in (N0+1):N2)
        {
            ...
            if(r_in(t, wkend_idx_local) == 1)
            {
                ...
                else if(SCHOOL_STATUS_local[t] == 0 && t>=elementary_school_reopening_idx_local)
                {
                    E_casesByAge[t] =
                        append_col(
                            tmp_row_vector_A_no_impact_intv[1:A_CHILD] * elt_school_intv_effect * cntct_elementary_school_reopening_weekends_local[1:A_CHILD,1:A_CHILD] +
                                tmp_row_vector_A[(A_CHILD+1):A] * cntct_elementary_school_reopening_weekends_local[(A_CHILD+1):A,1:A_CHILD],
                            tmp_row_vector_A * cntct_elementary_school_reopening_weekends_local[:,(A_CHILD+1):A] .* impact_intv[t,(A_CHILD+1):A]
                        );
                }
            ...
            else
            {
                ...
                else if(SCHOOL_STATUS_local[t] == 0 && t>=elementary_school_reopening_idx_local)
                {
                    E_casesByAge[t] =
                        append_col(
                            tmp_row_vector_A_no_impact_intv[1:A_CHILD] * elt_school_intv_effect * cntct_elementary_school_reopening_weekdays_local[1:A_CHILD,1:A_CHILD] +
                                tmp_row_vector_A[(A_CHILD+1):A] * cntct_elementary_school_reopening_weekdays_local[(A_CHILD+1):A,1:A_CHILD],
                            tmp_row_vector_A * cntct_elementary_school_reopening_weekdays_local[:,(A_CHILD+1):A] .* impact_intv[t,(A_CHILD+1):A]
                            );
                }
                else if(SCHOOL_STATUS_local[t] == 1 && t>=elt_reopening_index_local)
                {
        ...
        for (t in full_flows_Monday_idx_local:(full_flows_Monday_idx_local+n_days-1))
        {
            ...
            if(r_in(t, wkend_idx_local) == 1)
            {
                ...
                else if(SCHOOL_STATUS_local[t] == 0 && t>=elementary_school_reopening_idx_local)
                {
                    tmp_flow = append_col(
                        append_row(
                            rep_matrix(elt_school_intv_effect*(tmp_row_vector_A_no_impact_intv[1:A_CHILD])',A_CHILD),
                            rep_matrix((tmp_row_vector_A[(A_CHILD+1):A])', A_CHILD)
                            ),
                        rep_matrix(tmp_row_vector_A', A-A_CHILD)
                        )
            ...
            else
            {
                ...
                else if(SCHOOL_STATUS_local[t] == 0 && t>=elementary_school_reopening_idx_local)
                {
                    tmp_flow = append_col(
                        append_row(
                            rep_matrix(elt_school_intv_effect,A_CHILD,A_CHILD),
                            rep_matrix((tmp_row_vector_A[(A_CHILD+1):A])', A_CHILD)
                            ),
                        rep_matrix(tmp_row_vector_A', A-A_CHILD)
                        )
...
parameters {
  ...
  real<lower=0.01,upper=1.0> elt_school_intv_effect;
}
