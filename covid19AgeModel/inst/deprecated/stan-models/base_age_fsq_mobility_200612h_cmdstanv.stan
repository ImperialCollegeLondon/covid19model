...
generated quantities {
  ...
  for( m in 1:M )
  {
    ...
    // generate R_ta
    {
        ...
        // RtByAge[m] = prop_susceptibleByAge .* rep_matrix( exp(log_relsusceptibility_age), N2);
        RtByAge[m] = rep_matrix( append_col(exp(log_relsusceptibility_age),1.), N2);
        for(t in 1:N2)
        {
            if(r_in(t, wkend_idx[,m]) == 1)
            {
                RtByAge[m][t,:] = ( RtByAge[m][t,:] * (cntct_weekends_mean[m]') ) .* impact_intv[t,:];
            }
            else
            {
                RtByAge[m][t,:] = ( RtByAge[m][t,:] * (cntct_weekdays_mean[m]') ) .* impact_intv[t,:];
            }
        }
    }
