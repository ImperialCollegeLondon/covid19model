...
transformed data {
  ...
  matrix[num_basis, n_age_continuous] B;  // matrix of B-splines
  ...
  B[ind,:] = to_row_vector(build_b_spline(age_continuous, to_array_1d(ext_knots), ind, spline_degree + 1));
  B[num_knots + spline_degree - 1, n_age_continuous] = 1;
}
...
transformed parameters {
  ...
  vector[n_age_continuous] beta_age_cont;
  row_vector[A] beta_age;
   a[1] = a_raw[1];
  for (i in 2:num_basis)
    a[i] = a[i-1] + a_raw[i]*rho;
  beta_age_cont = a0*to_vector(age_continuous) + to_vector(a*B);
  beta_age = to_row_vector(get_age_discrete * beta_age_cont);
}
