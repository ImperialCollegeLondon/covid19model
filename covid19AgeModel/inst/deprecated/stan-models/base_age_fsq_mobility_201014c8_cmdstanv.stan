functions {
    ...
    matrix country_EdeathsByAge(// parameters
        ...
        real log_ifr_overall_upswing_rnde
        )
    {
        real zero = 0.0;
        ...
        log_ifr_age[(rebound_date_time_idx_local):N2] += rep_matrix( log_ifr_overall_upswing_rnde , N2 - rebound_date_time_idx_local + 1, A);
    ...
    for(m_slice in 1:M_slice) {
      ...
      E_deathsByAge =
        country_EdeathsByAge(
            ...
            log_ifr_overall_upswing_rnde[m]
            );
...
parameters {
  ...
  real<upper=0> log_ifr_overall_upswing_rnde[M];
  real<lower=0> sd_log_ifr_overall_upswing_rnde;
}
...
model {
  ...
  log_ifr_overall_upswing_rnde ~ normal(0, sd_log_ifr_overall_upswing_rnde);
  sd_log_ifr_overall_upswing_rnde ~ exponential(10);
