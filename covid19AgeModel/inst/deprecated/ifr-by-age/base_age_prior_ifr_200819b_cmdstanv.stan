...
transformed parameters
{
  ...
  for(i in 1:N)
    p_log_avg =  alpha + beta*age_study  + gamma[study[i]];
  ...
  for(n in 1:N){
    {
      ...
      vector[casesN_local] log_choose_fct_local = log_choose_fct[1:(casesU[n]-casesL[n]+1), n];
      vector[casesN_local] auxCases_local = auxCases[1:(casesU[n]-casesL[n]+1), n];
      target += log_sum_exp( log_choose_fct_local + rep_vector(deaths[n], casesN_local) .* log( exp(rep_vector(p_log_avg[n], casesN_local )) ) + 
              (auxCases_local-rep_vector(deaths[n], casesN_local)) .* log(1 - exp(rep_vector(p_log_avg[n], casesN_local))) );
    }
  }
