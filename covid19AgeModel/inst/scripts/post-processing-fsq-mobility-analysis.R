...
{	
	args_dir <- list()
	args_dir[['stanModelFile']] <- 'base_age_fsq_mobility_201015f8_cmdstanv'
	args_dir[['out_dir']] <- '/Users/or105/Downloads/base_age_fsq_mobility_201015f8_cmdstanv-40states_Oct29-464405'
	args_dir[['job_tag']] <- '40states_Oct29-464405'
	args_dir[['overwrite']] <- 0
}
...
ggsave(p, file=file, w=15, h=15)
ggsave(p, file=gsub('.png','.pdf',file), w=15, h=15,dpi=500)
...
plot.mobility.trend.ntl <- ggplot(fsq_a) +
	...
	plot.contacts.CA <-		ggplot(cnts, aes(y = age_cnt, x = age_index)) + 
			...
			theme_bw(base_size=20)   +
			scale_fill_viridis(begin=0,end=1,alpha=0.8,direction=1,option="viridis",values = scales::rescale(c(0,0.3,0.5,3,8)),breaks=seq(0,8,2)) + # same scale as exploratry plot
			guides(fill = guide_colourbar(barwidth = 7, barheight = 0.5, direction="horizontal")) +
	...
	cat("\nWrite to file",file,"...")
	ggsave(plot.overall, file=file, w=15, h=18)
	ggsave(gsub('.png','.pdf',file), plot.overall, w=15, h=18,dpi = 500)
