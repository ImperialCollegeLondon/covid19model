# post-processing-validate-attackrates.R
# 
###############################################################################
...
{
	args_dir <- list()
	args_dir[['stanModelFile']] <- 'base_age_fsq_mobility_200803c1_cmdstanv'
	args_dir[['out_dir']] <- '/rdsgpfs/general/project/ratmann_covid19/live/age_renewal_usa/base_age_fsq_mobility_200803c1_cmdstanv-4states_updateifrprior_under75'
	args_dir[['job_tag']] <- '4states_updateifrprior_under75'
...
cat("\n ----------- summarise_attackrate_byage_c overall ----------- \n")

#	summarise cases/attack rate overall
...
cat("\n ----------- summarise attackrate for 18plus population only ----------- \n")
#	summarise cases/attack rate 18plus population
file <- paste0(outfile.base,'-summary-cases-18plus.RDS')
if(!file.exists(file) | args_dir[['overwrite']])
{	
  ...
  e_acases_byage_s_18plus <- summarise_e_acases_byage_c(E_casesByAge,
                                                        age_cat_map_18plus, 
                                                        plot.pars.basic$pop_info, 
                                                        plot.pars.basic$dates, 
                                                        plot.pars.basic$regions)
  cat("\nWrite ",file," ... ")
  saveRDS(e_acases_byage_s_18plus, file=file)
}
if(file.exists(file))
{
  e_acases_byage_s_18plus <- readRDS(file)
}

attackrate_byage_s_18plus <- summarise_attackrate_byage_c(e_acases_byage_s_18plus, age_cat_map_18plus,
																															 plot.pars.basic$pop_info, plot.pars.basic$regions)
attackrate_byage_s_18plus <- attackrate_byage_s_18plus[!is.na(age_cat),-c(1:2)]


cat("\n ----------- summarise_attackrate_byage_c for NY study age bands and make validation table ----------- \n")
#	summarise cases/attack rate NY study age bands
file <- paste0(outfile.base,'-summary-cases-NYstudy.RDS')
if(!file.exists(file) | args_dir[['overwrite']])
{	
  if(is.null(E_casesByAge)){
    file2 <- paste0(outfile.base,'-stanout-E_casesByAge-gqs.RDS')
  ...
  e_acases_byage_s <- summarise_e_acases_byage_c(E_casesByAge,
                                                 age_cat_map_NYstudy, 
                                                 plot.pars.basic$pop_info, 
                                                 plot.pars.basic$dates, 
                                                 plot.pars.basic$regions)
  cat("\nWrite ",file," ... ")
  saveRDS(e_acases_byage_s, file=file)
}
if(file.exists(file))
{
  e_acases_byage_s <- readRDS(file)
}

attackrate_byage_s <- summarise_attackrate_byage_c(e_acases_byage_s, age_cat_map_NYstudy,
																									 plot.pars.basic$pop_info, plot.pars.basic$regions)
dates <- unique(nystudy$dates)
dates <- as.Date(dates)+25
# Calculate for date in paper (assuming lag of 25 days) and again for an assumed lag of 16 days
dateslag <- c(dates-25,dates-16)

artable <- make_attackrate_validation_table(attackrate_byage_s,attackrate_byage_s_18plus,plot.pars.basic$pop_info,dateslag,unique(nystudy$state),nystudy,outfile.base)
...
cat("\n ----------- summarise_attackrate_byage_c for CDC data ----------- \n")
#	summarise cases/attack rate NY study age bands
...
attackrate_byage_s <- summarise_attackrate_byage_c(e_acases_byage_s, age_cat_map_CDC,
																									 plot.pars.basic$pop_info, plot.pars.basic$regions)



# NB use over 18s summary for calculating overall attack rate for Florida in round 2
for(i in 1:2){
	...
	for(states in unique(study$state)){
		...
		dates <- as.Date(dates)
		dateslag <- c(dates-16,dates-25)
			if(states=='FL' & i==2){
			ans <- make_attackrate_validation_table(attackrate_byage_s,attackrate_byage_s_18plus,plot.pars.basic$pop_info,dateslag,states,study,outfile.base)
		}else	{
		ans <- make_attackrate_validation_table(attackrate_byage_s,attackrate_overall_c,plot.pars.basic$pop_info,dateslag,states,study,outfile.base)
		}
...
cat('\nWriting ',paste0(outfile.base,'-attackrate_validationtable.rds'),' ...')
saveRDS(artable, file = paste0(outfile.base,'-attackrate_validationtable.rds'), version = 2)
