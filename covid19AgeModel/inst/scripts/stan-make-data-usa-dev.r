...
args <- list( 
  stanModelFile= 'base_age_fsq_mobility_200929b9_cmdstanv',
  seed= 42,
  chain= 1,
  #outdir= "~/figures",
  outdir= "~/Downloads",
  job_tag= '4states_Sep29',
  cmdstan = 0L,
  multiplier_cntct_school_closure = 1,
  forecast_with_schools_reopened = 1,	#NOTE: in the dev code after report 32 this means "sinceAug24_with_schools_reopened"
  ifr_by_age_prior = "BetaBinomial_report32",
  #ifr_by_age_prior = "Levin_meta_analysis",
  #ifr_by_age_prior = "Verity_model_based_analysis",
...
if(length(args_line) > 0) 
{
  ...
  args[['ifr_by_age_prior']] <- as.character(args_line[[20]])
} 
...
args$file_jhu_death_data_padded <- file.path(pkg.dir,"data","jhu_death_data_padded_200926.rds")
args$file_nyt_death_data_padded <- file.path(pkg.dir,"data","nyt_death_data_padded_200926.rds")
args$file_nyc_death_data_padded <- file.path(pkg.dir,"data","NYC_deaths_200926.csv")
#args$file_death_data_by_age <- file.path(pkg.dir,"data","DeathsByAge_US_200910.csv")
args$file_death_data_by_age <- file.path(pkg.dir,"data","DeathsByAge_US_200926.csv")
...
args$file_fsq_mobility <- file.path(pkg.dir,'data','fsq_visit_data_sep_refresh_200920.csv')
