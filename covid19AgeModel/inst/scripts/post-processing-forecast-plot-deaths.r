...
args_dir <- list()
args_dir[['stanModelFile']] <- 'covid19AgeModel_v110_cmdstanv'
args_dir[['out_dir']] <- '/Users/melodiemonod/short_run_test/covid19AgeModel_v110_cmdstanv-40states_tau10_v110'
args_dir[['job_tag']] <- '40states_tau10_v110'
...
E_deathsByAge_school_reopen_1 <- NULL
...
file <- paste0(outfile.base,'-summary-deaths-overall-with-national-average',suffix_sensitivity_school0,'.RDS')
...
{	
  ...
  cat("\n ----------- make_deathsoverall_summaries_with_national_average ----------- \n")	
  deaths_school_reopen_0 <- make_deathsoverall_summaries_with_national_average(E_deathsByAge_school_reopen_0, 
...
file <- paste0(outfile.base,'-summary-deaths-overall-with-national-average',suffix_sensitivity_school1,'.RDS')
if(!file.exists(file) | args_dir[['overwrite']])
{	
  if(is.null(E_deathsByAge_school_reopen_1))
  {
    file2 <- paste0(outfile.base,'-stanout-E_deathsByAge-gqs',suffix_sensitivity_school1,'.RDS')
    cat("\n read RDS:", file2)
  	E_deathsByAge_school_reopen_1 <- readRDS(file2)
  }
  
  cat("\n ----------- make_deathsoverall_summaries_with_national_average ----------- \n")	
  deaths_school_reopen_1 <- make_deathsoverall_summaries_with_national_average(E_deathsByAge_school_reopen_1, 
	plot.pars.basic$dates, 
	plot.pars.basic$regions, 
	plot.pars.basic$pop_info)
  cat("\nWrite ",file," ... ")
...
deaths.plots <- plot_deaths_overall_overtime_forecast_school_reopen( 
	subset(deaths_school_reopen_1, loc != "US"), 
	subset(deaths_school_reopen_0, loc != "US"),
	cd_data, 
	plot.pars.basic$dates,
	plot.pars.basic$regions,
...
ggsave(paste0(outfile.base,'-excessdeathsper1e6_casesper1e6_overall', '_counterfactual_', args_dir$counterfactual.scenario, '_multiplier_', multiplier, '_level_', args_dir$school_level,'.png'), g, w = 12, h=12)
...
g <- ggplot(data= exd) +
		...
		scale_color_viridis_d(aesthetics = c("col"),direction=-1) + 
		guides(color = guide_legend(nrow=5,bycol=TRUE))
ggsave(paste0(outfile.base,'-excessdeathsper1e6_casesper1e6_30to49', '_counterfactual_', args_dir$counterfactual.scenario, '_multiplier_', multiplier, '_level_', args_dir$school_level,'.png'), g, w = 12, h=12)
...
{	
  ...
  # verify that we have enough sample before performing a regression
  if(dim(E_effcasesByAge)[1] < 2e3){
    cat("Not enough sample to perfom a regression on the results")
    quit()
  }
...
{	
  ...
  # verify that we have enough sample before performing a regression
  if(dim(E_deathsByAge_school_reopen_1)[1] < 2e3){
    cat("Not enough sample to perfom a regression on the results")
    quit()
  }
  ...
  samples_excess_deaths <- readRDS(file)
}
...
ggsave(paste0(outfile.base,'-excessdeathsper1e6_casesper1e6_significant_covariates', '_counterfactual_', args_dir$counterfactual.scenario, '_multiplier_', multiplier, '_level_', args_dir$school_level,'.png'), g, w = 4, h=7)
