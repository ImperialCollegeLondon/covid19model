...
if(0)
{
	...
	args_dir[['out_dir']] <- '~/Box\ Sync/2020/R0t/results/base_age_fsq_mobility_201015i4_cmdstanv-40states_Oct29_Levin7_schoolbound6_FSQupdate'
	args_dir[['job_tag']] <- '40states_Oct29_Levin7_schoolbound6_FSQupdate'	
...
file <- file.path(pkg.dir,"data","df_predict_reporting_age_strata_210103_cured.rds")
...
death.predict_subset = subset(death.predict, code %in% names(plot.pars.basic$dates))
g_deaths <- plot_proportion_monthly_death_by_age(subset(death.predict_subset, age_index != 0), plotdates, 1, T) +
  labs(x='', y='Proportion of COVID-19 monthly deaths', fill='Age band', col = 'Age band') 
g_obs_cases <- plot_crude_proportion_monthly_cases_by_age_withCI(death.predict_subset, "20-49",plotdates) + 
