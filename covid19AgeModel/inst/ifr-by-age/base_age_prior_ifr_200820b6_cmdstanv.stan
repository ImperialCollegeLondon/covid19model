functions {
        ...
        )
    {
        ...
        bb_alpha = inv_logit( logit_p_avg ) * inv(inv_overd);
        bb_beta = (1. - inv_logit( logit_p_avg )) * inv(inv_overd);
        ...
        {
            n = n_slice + start - 1;
            lpmf += beta_binomial_lpmf( deaths_slice[n] | casesM[n], bb_alpha[n], bb_beta[n]);
        }
        return( lpmf );
    }
}
...
model
{
    target += cauchy_lpdf( inv_overd | 0, 1);
...
generated quantities
{
    ...
    real obs_pred[N_predict];
    
    obs_pred = normal_rng(rep_array(0, N_predict), rep_array(obs_rnde_sd, N_predict));
    p_predict = inv_logit( alpha + gp_age_predict + beta * age_predict + to_vector(obs_pred));
    
    {
        vector[N_predict] bb_alpha;
        vector[N_predict] bb_beta;
        
        bb_alpha = p_predict * inv(inv_overd);
        bb_beta = (1-p_predict) * inv(inv_overd);

        pred_deaths = beta_binomial_rng(rep_array(10000000, N_predict), bb_alpha, bb_beta);
    }
}
