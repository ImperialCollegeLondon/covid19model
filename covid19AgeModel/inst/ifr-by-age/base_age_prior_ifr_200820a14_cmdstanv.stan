...
transformed parameters
{
    ...
    //var_infl = exp( alpha0 + alpha1 * age_study );
...
generated quantities
{
    ...
    //vector[N_predict] overd_predict;
    int pred_deaths[N_predict];
    
    p_predict = inv_logit( beta0 + beta1 * age );
    //overd_predict = exp( alpha0 + alpha1 * age );
    
    {
        vector[N_predict] alpha;
        vector[N_predict] beta;
        alpha = p_predict * overd;
        beta = (1-p_predict) * overd;
