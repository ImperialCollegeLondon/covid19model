...
matrixStats::colQuantiles(data.matrix(first.intervention),probs=c(.025,.975))
...
p = ggplot(data) +ggpubr::theme_pubr() +  geom_point(aes(x=m,y=parameter,colour=type),position = position_dodge(-.5)) + 
