...
source("nature/utils/geom-stepribbon.r")
#---------------------------------------------------------------------------
make_three_panel_plot <- function(filename){
  
  print(sprintf("loading: %s",paste0("nature/results/",filename,'-stanfit.Rdata')))
  load(paste0("nature/results/", filename,'-stanfit.Rdata'))
  out <- rstan::extract(fit)
  covariates = readRDS('nature/data/interventions.rds')
...
make_plots <- function(data_country, covariates_country_long, 
  ...
  save_plot(filename = paste0("nature/figures/", country, "-three-pannel-", filename, ".png"),
            p, base_width = 14)
}
