...
process_covariates <- function(states, mobility, intervention = NULL, 
                               interventions_lifted  = readRDS('usa/data/covariates_all_ended.RDS'),
  ...
  f_cached2 <- ecdf(x1) # empirical cumulative distribtion function
  ...
  colnames(interventions_lifted) <- c("StatePostal_lif", "CaseIsolation_lif", "EmergDec_lif", "GathRestrict_lif", 
                                      "NEBusinessClose_lif", "OtherBusinessClose_lif", "PublicMask_lif", 
                                      "Quarantine_lif", "RestaurantRestrict_lif", "SchoolClose_lif", "StayAtHome_lif", 
                                      "TravelRestrictEntry_lif",  "TravelRestrictExit_lif", "TravelRestrictIntra_lif")
  ...
  for(State in states) {
    ...
    intervention_state <- interventions[interventions$StatePostal == State, c(2:14)]
    # Selects correct Interventions Lifted for each region
    intervention_lifted_state <- interventions_lifted[interventions_lifted$StatePostal == State, c(2:15)]
    ...
    f2 = rep(0,num_days_sim) # f is the probability of dying on day i given infection
    ...
    # creating features
    df_features <- create_features(len_mobility, padded_covariates, covariates_forecast, 
                                   covariates_lifted_forecast, transit_usage)
  ...
  for(state.i in 1:stan_data$M) {
    ...
    last_ar_week = which(dates[[state.i]]==max(death_data$date)) - 7
...
create_features <- function(len_mobility, padded_covariates, covariates_forecast, covariates_lifted_forecast, transit_usage){
    return (data.frame('caseIsolation' = covariates_forecast$CaseIsolation, 'emergency' = covariates_forecast$EmergDec,
                       'gatherRestrict' = covariates_forecast$GathRestrict,
                       'neBusiness' = covariates_forecast$NEBusinessClose, 
                       'otherBusiness' = covariates_forecast$OtherBusinessClose,
                       'mask' = covariates_forecast$PublicMask, 'quarantine' = covariates_forecast$Quarantine,
                       'restaurant' = covariates_forecast$RestaurantRestrict, 'school' = covariates_forecast$SchoolClose, 
                       'stayHome' = covariates_forecast$StayAtHome, 'travelEntry' = covariates_forecast$TravelRestrictEntry,
                       'travelExit' = covariates_forecast$TravelRestrictExit, 'travelIn' = covariates_forecast$TravelRestrictIntra,
                       'firstIntervention' = 1*((covariates_forecast$CaseIsolation + 
                                                   covariates_forecast$EmergDec + 
                                                   covariates_forecast$GathRestrict + 
                                                   covariates_forecast$NEBusinessClose + 
                                                   covariates_forecast$OtherBusinessClose + 
                                                   covariates_forecast$PublicMask + 
                                                   covariates_forecast$Quarantine + 
                                                   covariates_forecast$RestaurantRestrict + 
                                                   covariates_forecast$SchoolClose + 
                                                   covariates_forecast$StayAtHome + 
                                                   covariates_forecast$TravelRestrictEntry + 
                                                   covariates_forecast$TravelRestrictExit + 
                                                   covariates_forecast$TravelRestrictIntra) >= 1),
                       'caseIsolation_lifted' = covariates_lifted_forecast$CaseIsolation, 
                       'emergency_lifted' = covariates_lifted_forecast$EmergDec,
                       'gatherRestrict_lifted' = covariates_lifted_forecast$GathRestrict,
                       'neBusiness_lifted' = covariates_lifted_forecast$NEBusinessClose, 
                       'otherBusiness_lifted' = covariates_lifted_forecast$OtherBusinessClose,
                       'mask_lifted' = covariates_lifted_forecast$PublicMask, 
                       'quarantine_lifted' = covariates_lifted_forecast$Quarantine,
                       'restaurant_lifted' = covariates_lifted_forecast$RestaurantRestrict, 
                       'school_lifted' = covariates_lifted_forecast$SchoolClose, 
                       'stayHome_lifted' = covariates_lifted_forecast$StayAtHome, 
                       'travelEntry_lifted' = covariates_lifted_forecast$TravelRestrictEntry,
                       'travelExit_lifted' = covariates_lifted_forecast$TravelRestrictExit, 
                       'travelIn_lifted' = covariates_lifted_forecast$TravelRestrictIntra,
                       "April1" = covariates_lifted_forecast$April1,
                       'firstIntervention_lifted' = 1*((covariates_lifted_forecast$CaseIsolation_lif + 
                                                          covariates_lifted_forecast$EmergDec_lif + 
                                                          covariates_lifted_forecast$GathRestrict_lif + 
                                                          covariates_lifted_forecast$NEBusinessClose_lif + 
                                                          covariates_lifted_forecast$OtherBusinessClose_lif + 
                                                          covariates_lifted_forecast$PublicMask_lif + 
                                                          covariates_lifted_forecast$Quarantine_lif + 
                                                          covariates_lifted_forecast$RestaurantRestrict_lif + 
                                                          covariates_lifted_forecast$SchoolClose_lif + 
                                                          covariates_lifted_forecast$StayAtHome_lif + 
                                                          covariates_lifted_forecast$TravelRestrictEntry_lif + 
                                                          covariates_lifted_forecast$TravelRestrictExit_lif + 
                                                          covariates_lifted_forecast$TravelRestrictIntra_lif) >= 1),
